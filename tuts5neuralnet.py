...
# such as CUDA, MPS, MTIA, or XPU. If unavailable, the CPU will be used.
...
# To illustrate it, we will take a sample minibatch of 3 images of size 28 x 28.
# See what happens to it as we pass it through the neural network.
...
# Initialize nn.Flatten layer to convert each 2D 28 x 28 image into a contiguous array
# of 784 pixel values (the minibatch dimension (at dim=0) in maintained).
...
# which helps the neural networks learn a wide variety of phenomena.
...
# The sequential module is an ordered container of modules.
...
# which are passed to the layer nn.Softmax module.
# The logits are scaled to values [0, 1] representing the model's predicted probabilities for each class.
# The parameter, dim, indicates the dimension along which the values must sum to 1.
...
# The associated weights and biases that are optimized during training.
